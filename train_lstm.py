...
from models import Seq2SeqLSTM, QuantileLoss

def train_lstm_model(data_path="data/processed/features_data.parquet", models_dir="models"):
    """
    Trains the Seq2Seq LSTM model using Quantile Loss.
    """
    ...
    # 1. Prepare Datasets
    print("Preparing sequence datasets for LSTM...")
    ...
    model = Seq2SeqLSTM(num_features=num_features, d_model=64, n_horizons=3, n_quantiles=3)
    ...
    # MLflow tracking
    try:
        mlflow.set_experiment("ISRO_Space_Weather_LSTM")
        mlflow.start_run(run_name="lstm_seq2seq_training")
        mlflow.log_params({
    ...
    for epoch in range(epochs):
        ...
        for idx, (batch_x, batch_y) in enumerate(train_loader):
            ...
            optimizer.zero_grad()
            preds = model(batch_x) # (batch, n_horizons, n_quantiles)
